...
class TimeSync:
    ...
    def __init__(self):
        ...
        self.min_query_interval = 60 / 1200  # 每分钟1200次请求限制
    ...
    async def _final_time_sync(self) -> Tuple[float, float]:
        """最终时间同步"""
        try:
            latencies = []
            offsets = []
            ...
            for _ in range(5):
                ...
                server_time = await self.get_binance_time()
...
""")
    ...
    async def get_binance_time(self) -> int:
        """获取币安服务器时间"""
        try:
...
class BinanceSniper:
    ...
    async def snipe(self):
        ...
        try:
            ...
            # 3. 同步时间
            sync_results = await self.time_sync.sync()
            if not sync_results:
                self.logger.error("时间同步失败")
                return None
                
            # 4. 委托给 OrderProcessor 执行订单
            return await self.order_processor.execute_orders_with_strategy()
            
        except Exception as e:
            self.logger.error(f"抢购执行失败: {str(e)}")
            return None

    async def _wait_for_opening(self):
        """等待开盘"""
        self.logger.info(f"""
=== 等待开盘 ===
交易对: {self.symbol}
