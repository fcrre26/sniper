...
class BinanceSniper:
    ...
    def __init__(self, config: ConfigManager):
        """初始化基础配置"""
        self.config = config
        self._resources = set()  # 用于跟踪需要清理的资源
        ...
        self.api_permissions = "现货交易"
        self.risk_level = "中等"
        self.max_slippage = 0.1
        self.max_price_gap = 0.2
    ...
    def initialize_trading_system(self):
        ...
        try:
            ...
            if not self.symbol or not self.amount or not self.opening_time:
                logger.error("缺少必要的交易参数")
                print("请先完成交易策略设置")
                return False
            
            # 2. 初始化客户端
            if not self._init_clients():
                logger.error("初始化交易客户端失败")
                print("初始化交易客户端失败，请检查API配置")
                return False
            ...
            # 4. 同步时间
            try:
                await self.time_sync.sync(self.query_client)
                logger.info("时间同步完成")
            except Exception as e:
                logger.error(f"时间同步失败: {str(e)}")
                return False
...
""")
    ...
    async def get_market_price(self) -> float:
        ...
        try:
            ...
            self.best_bid_qty = ticker['bidVolume']
            self.best_ask_qty = ticker['askVolume']
...
- 滑点限制: {self.max_slippage*100}%
- 深度要求: {self.min_depth_requirement}档

⚡ 执行策略
- 并发订单: {self.concurrent_orders}个
- 提前时间: {self.advance_time}ms
- 重试次数: {self.retry_attempts}次
- 网络延迟: {self.network_latency:.2f}ms
...
2. 已启用深度检查: 最小{self.min_depth_requirement}档
3. 已启用断层保护: {self.max_price_gap*100}%
4. 已启用滑点控制: {self.max_slippage*100}%
5. 使用IOC限价单模式
""")
    ...
    def load_strategy(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                # 设置默认值
                self.max_slippage = 0.1  # 10%的最大滑点
                self.max_price_gap = 0.2  # 20%的价格断层保护
                self.min_depth_requirement = 5  # 最小深度要求5档
                self.retry_attempts = 3  # 重试3次
                self.network_latency = 0.0  # 初始网络延迟
                self.time_offset = 0.0  # 初始时间偏移
        ...
        while True:
            try:
                ...
                if choice == "0":
                    ...
                    # 打印策略并等待用户确认
                    success = sniper.print_strategy()
                    # 删除这里的日志记录，只保留 print_strategy 方法内的记录
                    input("\n按回车返回主菜单...")
                    continue
                elif choice == "4":
